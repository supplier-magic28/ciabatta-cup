...
export async function loadPersonalCalendar(session: SessionPlayer, today = dateKeyInZone(new Date()), now = new Date().toISOString()): Promise<CalendarData> {
  ...
  const [playersResult, matchesResult, externalResult, practicesResult, plansResult, entriesResult, tournamentsResult, placementsResult, tournamentMatchesResult] = await Promise.all([
    ...
    db.from("tournaments").select("id,name,status,starts_at,location_name,court_id,default_surface,courts(name)"),
  ...
  for (const match of matchesResult.data ?? []) {
    ...
    events.push({
      ...
      title: `vs ${external ? (externalNames.get(match.id) ?? "Non-Ciabatta opponent") : (names.get(opponentId ?? "") ?? "Opponent")}`,
      ...
      score: formatScore(sets), metadataMissing: !match.court_id || !match.surface,
    });
  }

  for (const practice of practicesResult.data ?? []) events.push({
    ...
    won: null, surface: null, court: "Not recorded", location: null, score: null, metadataMissing: false,
  });
  ...
  for (const plan of plansResult.data ?? []) {
    ...
    events.push({ key: `planned:${plan.id}`, kind: "planned", sourceId: plan.id, date: dateKeyInZone(plan.scheduled_at), startsAt: plan.scheduled_at, title: `vs ${opponent}`, subtitle: plan.status === "proposed" ? "Awaiting reply" : "Locked in", href: `/matches/${plan.id}`, status: plan.status === "proposed" ? "awaiting_reply" : plan.scheduled_at >= now ? "future" : "past", points: 0, won: null, surface: null, court: one(plan.courts)?.name ?? null, location: plan.location || null, score: null, metadataMissing: false });
  }
  ...
  for (const tournament of (tournamentsResult.data ?? []).filter((row) => entered.has(row.id))) {
    ...
    events.push({ key: `cup:${tournament.id}`, kind: "cup", sourceId: tournament.id, date: dateKeyInZone(tournament.starts_at), startsAt: tournament.starts_at, title: tournament.name, subtitle: `${won}-${fixtures.length - won} fixtures · ${ordinal(placement?.placement ?? null)}`, href: `/tournaments/${tournament.id}`, status: tournament.status === "completed" || dateKeyInZone(tournament.starts_at) < today ? "past" : "future", points: pointsBySource.get(tournament.id) ?? placement?.points ?? 0, won: placement?.placement === 1 ? true : null, surface: tournament.default_surface, court: one(tournament.courts)?.name ?? null, location: tournament.location_name, score: null, metadataMissing: false, placement: placement?.placement ?? null, record: { won, lost: fixtures.length - won } });
  }
