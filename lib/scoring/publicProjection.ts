...
import { buildRatingCache, type ScoringMatchRow, type TournamentPlacementRow } from "./materialization";
...
export async function loadPublicLadderProjection(playerIds: string[]) {
  ...
  const [matches, placements, practices, playDays] = await Promise.all([
    ...
    db.from("tournament_placements").select("tournament_id,player_id,placement,points,awarded_at"),
  ...
  const placementRows = (placements.data ?? []) as Array<TournamentPlacementRow & { tournament_id: string; placement: number }>;
  return {
