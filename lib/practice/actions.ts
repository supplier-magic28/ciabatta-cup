...
export async function submitPractice(_previous: PracticeActionState | undefined, formData: FormData): Promise<PracticeActionState> {
  ...
  try { await sendTournamentEmail(player.email, renderPracticeEmail({ kind: "logged", firstName: player.firstName ?? "Player", activity: checked.value.activity, minutes: checked.value.minutes, practiceDate: checked.value.practicedOn }), `practice/logged/${data.id}/${player.id}`); }
...
export async function reviewPractice(id: string, decision: "approved" | "rejected"): Promise<{ ok: boolean; error?: string }> {
  ...
  if (!practice || practice.status !== "pending") return { ok: false, error: "This practice is no longer awaiting review." };
  const { error } = await supabase.from("practice_sessions").update({ status: decision, reviewed_by: reviewer.id, reviewed_at: new Date().toISOString() }).eq("id", id).eq("status", "pending");
  if (error) return { ok: false, error: "Couldn't review that practice." };
  if (decision === "approved") try { await rebuildRatingCache(); } catch { return { ok: false, error: "Practice approved, but points need an admin rebuild." }; }
  const owner = Array.isArray(practice.players) ? practice.players[0] : practice.players;
  try { if (owner?.email) await sendTournamentEmail(owner.email, renderPracticeEmail({ kind: decision, firstName: owner.first_name ?? "Player", activity: practice.activity, minutes: practice.minutes, practiceDate: practice.practiced_on }), `practice/${decision}/${id}/${practice.player_id}`); } catch { /* committed review remains successful */ }
