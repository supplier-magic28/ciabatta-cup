...
const refresh = () => ["/", "/matches", "/profile", "/admin/approvals"].forEach((path) => revalidatePath(path));
const note = async (playerId: string, kind: "match_proposed"|"match_declined"|"match_cancelled"|"match_locked_in"|"result_to_approve"|"result_confirmed", plannedMatchId: string, body: string) => createAdminClient().from("notifications").insert({ player_id: playerId, kind, planned_match_id: plannedMatchId, body });

export async function planMatch(input: { opponentPlayerId?: string; opponentExternalId?: string; scheduledAt: string; location: string }): Promise<Result> {
...
export async function markNotificationsRead(){const player=await getSessionPlayer();if(!player)return;const db=await createClient();await db.from("notifications").update({read_at:new Date().toISOString()}).eq("player_id",player.id).is("read_at",null);revalidatePath("/profile");}
