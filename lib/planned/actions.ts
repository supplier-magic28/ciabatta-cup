...
type Result = { ok: true; id?: string; warning?: string; deliveryWarning?: CommittedEmailWarning } | { ok: false; error: string };
...
export async function planMatch(input: { operationKey?: string; opponentPlayerId?: string; opponentExternalId?: string; scheduledAt: string; location: string; courtId?: string }): Promise<Result> {
  ...
  if (player.status !== "active") return { ok:false, error:"You need an active account to plan a match." };
  ...
  const deliveryWarning = external ? await sendLifecycleEmail(data, "locked") : undefined;
  refresh(); return { ok:true, id:data, deliveryWarning };
}

export async function decidePlannedMatch(id:string, decision:"accept"|"decline"|"cancel"):Promise<Result> {
  const player=await getSessionPlayer(); if(!player) return {ok:false,error:"Sign in first."}; if(player.status!=="active")return {ok:false,error:"You need an active account."}; const db=await createClient();
  ...
  const deliveryWarning=decision==="accept"?await sendLifecycleEmail(id,"locked"):undefined;
  refresh();return {ok:true,deliveryWarning};
}
...
export async function approvePlannedResult(id:string):Promise<Result>{const player=await getSessionPlayer();if(!player)return {ok:false,error:"Sign in first."};if(player.status!=="active")return {ok:false,error:"You need an active account."};const db=await createClient();const {data,error}=await db.rpc("approve_planned_result_v2",{p_planned_match_id:id});if(error)return {ok:false,error:error.message??"This result cannot be approved."};const row=Array.isArray(data)?data[0]:data;let warning:string|undefined;let deliveryWarning:CommittedEmailWarning|undefined;if(row?.match_status==="approved"&&row.match_id){warning=await rebuildScoringAfterCommit(row.match_id,"approve_planned_result");deliveryWarning=await sendLifecycleEmail(id,"confirmed",row.match_id);}refresh();return {ok:true,id:row?.match_id,warning,deliveryWarning};}

export async function requestPlannedResultCorrection(id:string):Promise<Result>{const player=await getSessionPlayer();if(!player)return {ok:false,error:"Sign in first."};if(player.status!=="active")return {ok:false,error:"You need an active account."};const db=await createClient();const {error}=await db.rpc("request_planned_result_correction_v2",{p_planned_match_id:id});if(error)return {ok:false,error:error.message??"Couldn't request a correction."};refresh();return {ok:true};}

export async function correctPlannedResult(id: string, input: MatchSubmission): Promise<Result> {
  const player = await getSessionPlayer();
  if (!player || player.role !== "admin" || player.status !== "active") {
    return { ok:false, error:"Only active organisers may correct results." };
  }
  const db = await createClient();
  const [{ data:plan }, { data:prior }] = await Promise.all([
    db.from("planned_matches").select("created_by,opponent_player_id").eq("id",id).single(),
    db.from("planned_match_results").select("submitted_by").eq("planned_match_id",id).order("created_at",{ascending:false}).limit(1).single(),
  ]);
  if (!plan || !plan.opponent_player_id || !prior) return { ok:false, error:"Result proposal not found." };
  const other = prior.submitted_by===plan.created_by ? plan.opponent_player_id : plan.created_by;
  const checked = validateSubmission({ ...input, opponentId:other }, prior.submitted_by);
  if (!checked.ok) return checked;
  let courtId: string | null = null;
  if (checked.value.location) {
    const { data:resolved, error:courtError } = await db.rpc("resolve_court",{ p_name:checked.value.location });
    if (courtError || !resolved) return { ok:false, error:"Couldn't save that court." };
    courtId=resolved as string;
  }
  const result=checked.value;
  const { error } = await db.rpc("correct_planned_result_v2",{
    p_planned_match_id:id,p_match_type:result.type,p_format:result.format,
    p_format_note:result.formatNote,p_winner_player_id:result.winnerId,
    p_score:result.sets,p_played_at:result.playedAt,p_location:result.location,
    p_court_id:courtId,p_surface:result.surface,
  });
  if (error) return { ok:false, error:error.message??"Couldn't save the correction." };
  refresh();
  return { ok:true };
}
...
export async function sendLifecycleEmail(
  ...
  try {
    ...
    return committedEmailWarning(
      "The match change committed, but its email needs recovery.",
      [],
    );
  }
}
