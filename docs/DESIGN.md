...
| 07 Manage tournament | `/admin/tournaments/new`, `/admin/tournaments/[tournamentId]` | Implemented | Photo-first partial creation, 2–8 ordered seats with persisted players shown in their saved seeds, reversible schedule lock, independent formats, three championship paths, atomic draw lock, and admin pre-play unlock for last-minute field edits are live. The unlock control is usable only while the locked scheduled cup has no match row; the database independently enforces the same boundary and also refuses placement facts. After a completed four-player group stage and before championship scoring, the director may explicitly select two finalists, record a reason, preserve the unplayed decider as visibly skipped, and install one best-of-three final; the control states that remaining players keep table order as third/fourth. Multi-set scoring, ordinary deciders/finals, and recovery-aware email controls are live. |
