...
| My trophies | `/tournaments`, `/tournaments/[tournamentId]/trophy` | Implemented | The signed-in player always sees a derived wooden trophy case between upcoming cups and the archive. Named and generic ranked wins remain distinct, repeat wins stay on the shelf, and owned `?trophy={tournamentId}` deep links open an accessible campaign detail sheet with event-local metadata, preserved cover crop, opponent avatars, and player-oriented scores. Trophy selection brightens on hover, focus, and press, then completes its visible shake before opening; optional synthesized cabinet sounds are on by default and persistently mutable. Registered Claymore wins enable an owned full-screen 3D viewer with a chronological derived engraving ledger and Android WebXR/Scene Viewer placement. Generic trophies remain 2D and iPhone Quick Look remains disabled pending physical-device testing. |
...
- A physical trophy's stable `trophy_key` selects its registered, versioned 3D
  asset and joins completed first-place facts into one oldest-first engraving
  ledger. Only owned awards with registered assets may open the viewer.
