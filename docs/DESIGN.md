...
| 07 Manage tournament | `/admin/tournaments/new`, `/admin/tournaments/[tournamentId]` | Partial | Four-player setup, participant replacement, deterministic fixtures, draw locking, lifecycle email actions, reviewed result entry, optional standings completion, deciders, finals, and stable pending states are live. Knockout preview and mid-event roster changes are deferred. |
