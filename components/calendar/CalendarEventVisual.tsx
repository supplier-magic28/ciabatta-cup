...
export function CalendarEventVisual({ event, size = "card" }: { event: CalendarEvent; size?: "compact" | "card" | "hero" }) {
  ...
  if (event.kind === "cup") return <div className={`relative overflow-hidden border-2 border-ink bg-green ${frame}`}>
    {event.coverImageUrl ? <Image src={event.coverImageUrl} alt={`${event.title} cover`} fill sizes={size === "hero" ? "(max-width: 768px) 100vw, 760px" : "(max-width: 768px) 100vw, 300px"} className="object-cover" style={{transform:`translate(${(event.coverCrop?.offsetX??0)/2}%,${(event.coverCrop?.offsetY??0)/2}%) scale(${event.coverCrop?.zoom??1})`}}/> : <div className="flex h-full items-center justify-center px-3 text-center font-heading text-sm font-bold text-chartreuse">CIABATTA CUP</div>}
  </div>;
