...
export function PracticeApprovalActions({ id }: { id: string }) {
  ...
  const run = (decision: "approved"|"rejected") => start(async () => { const result = await reviewPractice(id, decision); if (!result.ok) setError(result.error ?? "Review failed."); else setError(result.warning ?? result.deliveryWarning?.message ?? ""); });
