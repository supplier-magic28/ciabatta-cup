...
export function PlannedActions({id,invited,status}:{id:string;invited:boolean;status:string}){
  ...
  const run=(decision:"accept"|"decline"|"cancel")=>{setError("");start(async()=>{const result=await decidePlannedMatch(id,decision);if(!result.ok)setError(result.error);else{setError(result.deliveryWarning?.message??"");router.refresh();}});};
