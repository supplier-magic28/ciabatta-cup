...
export function TournamentCoverComposer({onPrepared,onPreparingChange}:{onPrepared:(file:File|null)=>void;onPreparingChange:(preparing:boolean)=>void}){
  const [url,setUrl]=useState<string|null>(null);const [shape,setShape]=useState<TournamentFrameShape>("wide");
  const [zoom,setZoom]=useState(1);const [offset,setOffset]=useState({x:0,y:0});const [error,setError]=useState<string|null>(null);const [preparing,setPreparing]=useState(false);const drag=useRef<{x:number;y:number;ox:number;oy:number}|null>(null);
  ...
  return <section className="order-first border-2 border-ink bg-row p-4 shadow-[4px_4px_0_var(--color-green)]">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
      <input type="file" accept="image/jpeg,image/png,image/webp" className="w-full font-mono text-[10px]" disabled={preparing} onChange={async(event)=>{const file=event.target.files?.[0];if(!file)return;if(!isAllowedTournamentPhoto(file)){setError("Choose a JPEG, PNG, or WebP photo under 5 MB.");return}setPreparing(true);onPreparingChange(true);setError(null);try{const prepared=await createTournamentSourceFile(file);if(url)URL.revokeObjectURL(url);setUrl(URL.createObjectURL(prepared));onPrepared(prepared)}catch(reason){setError(reason instanceof Error?reason.message:"That photo could not be prepared.")}finally{setPreparing(false);onPreparingChange(false)}}}/>
    ...
    {preparing&&<p className="mt-2 font-mono text-[10px] text-crust">Preparing upload…</p>}{error&&<p className="mt-2 font-mono text-[10px] text-rust" role="alert">{error}</p>}
