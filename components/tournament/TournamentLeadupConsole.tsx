...
import {buildOrderedTournamentRoster,includePersistedTournamentPlayers,type TournamentRosterParticipant} from "@/lib/tournament/roster";
...
export function TournamentLeadupConsole({tournament,participants,players}:{tournament:{id:string;starts_at:string;location_name:string;courts:number;default_surface:string|null;seat_count:number;schedule_locked_at:string|null;group_ruleset:TournamentRuleset;playoff_ruleset:TournamentRuleset;championship_path:TournamentChampionshipPath;cover_image_url:string|null};participants:TournamentRosterParticipant[];players:Player[]}){
  ...
  const [seats,setSeats]=useState(tournament.seat_count);const [roster,setRoster]=useState<string[]>(()=>buildOrderedTournamentRoster(tournament.seat_count,participants));
  const playerOptions=includePersistedTournamentPlayers(players,participants);
